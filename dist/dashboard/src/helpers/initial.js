...
import { getLanguage, logActions, logError, random } from "../utils/utils.js";
...
async function initialData({ anchorElement = document.body }) {
  try {
    async function initialSettings() {
      ...
      //get max group
      let maxGroup = await GM_getValue(KEY_MAX_GROUP_PER_TIME);
      if (!maxGroup) {
        maxGroup = MAX_GROUP_PER_TIME_INITIAL;
        GM_setValue(KEY_MAX_GROUP_PER_TIME, maxGroup);
      }
      ...
      const isFixStealFocus =
        (await GM_getValue(KEY_IS_FIX_STEAL_FOCUS)) || false;
      ...
      const isReloadDashboard =
        (await GM_getValue(KEY_IS_RELOAD_DASHBOARD)) || false;
      setIsReloadDashboard(isReloadDashboard);
    }

    await initialSettings();

    const listGroups = (await GM_getValue(KEY_GROUPS_NEED_POST)) || [];
    ...
    //...
    const gp = await GM_getValue(KEY_GROUPS_POSTED);
    if (!gp) {
      GM_setValue(KEY_GROUPS_POSTED, []);
    }

    const isDevMode = await GM_getValue(KEY_IS_DEVELOPER_MODE);
    if (isDevMode) {
