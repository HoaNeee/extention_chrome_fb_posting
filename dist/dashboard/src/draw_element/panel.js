...
import { openNewTask } from "../helpers/post.js";
import {
      ...
      <div style="overflow: auto; padding-right: 18px;" class="${prefix}div-dashboard">
        ...
        <div style="display: flex; gap: 8px; flex-wrap: wrap; flex-direction: column;">
            <div style="display: flex; gap: 4px;">
              <button button id="${prefix}btn-goto-dashboard" style="width: 100%;">${getTextWithLanguage({ vi: "Bảng điều khiển", en: "Goto Dashboard" })}</button>
  ...
  const advancedSettingHTML = `
    <div class="${prefix}advanced-setting" style="padding-left: 10px; overflow: auto; padding-right: 18px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 4px;">
          ...
          <div class="${prefix}field-container field-checkbox">
            <input type="checkbox" id="${prefix}checkbox-is-reload-dashboard">
            <label for="${prefix}checkbox-is-reload-dashboard" style="user-select: none;">${getTextWithLanguage({ vi: "Tự động tải lại trang", en: "Auto reload dashboard" })}</label>
          </div>
          <div class="${prefix}field-container field-checkbox">
          ...
          <div id="${prefix}div-scheduler-options" style="padding-left: 16px; max-width: 250px; min-width: 200px;">
            <div style="margin-bottom: 4px;">
              ...
              <button class="not-style" style="padding: 4px; font-size: 10px" id="${prefix}btn-hide-scheduler">${getTextWithLanguage({ vi: "Ẩn", en: "Hide options" })}</button>
            </div>
            <div id="${prefix}div-scheduler-setting" style="margin-top: 4px;">
              ...
              <div id="${prefix}div-scheduler-daily-hours" style="display: none; padding: 4px; margin-top: 4px;">
                <div style="text-align: right;">
                  <button class="not-style" style="padding: 4px; font-size: 10px; " id="${prefix}btn-reset-daily-hours">${getTextWithLanguage({ vi: "Đặt lại hàng ngày", en: "Reset daily hours" })}</button>
                </div>
              </div>
              <div id="${prefix}div-scheduler-custom-minutes" style="display: none; padding: 4px; margin-top: 4px;">
                <div class="${prefix}field-container">
                  ...
                  <div style="text-align: right;">
                    <button class="not-style" style="padding: 4px; font-size: 10px; " id="${prefix}btn-save-custom-minutes">${getTextWithLanguage({ vi: "Lưu", en: "Save" })}</button>
                  </div>
                </div>
              </div>
              <div id="${prefix}div-scheduler-custom-hours" style="display: none; padding: 4px; margin-top: 4px;">
                <div class="${prefix}field-container">
                  ...
                  <div style="text-align: right;">
                    <button class="not-style" style="padding: 4px; font-size: 10px; " id="${prefix}btn-save-custom-hours">${getTextWithLanguage({ vi: "Lưu", en: "Save" })}</button>
                  </div>
                </div>
              </div>
          ...
          <div style="margin-top: 8px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 4px;">
              <div class="${prefix}field-container">
                <label for="${prefix}input-delay-click-to-post" style="font-size: 11px">${getTextWithLanguage({ vi: "Chọn thời gian chờ nhấn nút hiển thị hộp thoại đăng", en: "Enter delay click to post" })} (seconds): </label>
              ...
              <div class="${prefix}field-container">
                <label for="${prefix}input-delay-fill-content" style="font-size: 11px">${getTextWithLanguage({ vi: "Chọn thời gian chờ điền nội dung", en: "Enter delay fill content" })} (seconds): </label>
              ...
              <div class="${prefix}field-container">
                <label for="${prefix}input-delay-fill-file" style="font-size: 11px">${getTextWithLanguage({ vi: "Chọn thời gian chờ điền tệp/file", en: "Enter delay fill file" })} (seconds): </label>
              ...
              <div class="${prefix}field-container">
                <label for="${prefix}input-delay-post" style="font-size: 11px">${getTextWithLanguage({ vi: "Chọn thời gian chờ nhấn nút đăng bài", en: "Enter delay post" })} (seconds): </label>
                <input min="1" style="font-size: 12px;" type="number" id="${prefix}input-delay-post" class="${prefix}input-outline" placeholder="Ex: 1,5,10,...">
              </div>
              <div class="${prefix}field-container">
                <label for="${prefix}input-delay-open-new-tab" style="font-size: 11px">${getTextWithLanguage({ vi: "Chọn thời gian chờ mở tab mới", en: "Enter delay open new tab" })} (seconds): </label>
...
async function createPanel(doc = document.body) {
  try {
    ...
    const dialogContainerElement = dialogContainer({ anchorElem: root });
    ...
    async function drawListGroups(data) {
      ...
      async function onDeleteGroup(id) {
        try {
          ...
          const indexsChecked =
            (await GM_getValue(KEY_INDEXS_GROUP_CHECKED)) || [];
          const set = new Set(indexsChecked);
          set.delete(id);
          GM_setValue(KEY_INDEXS_GROUP_CHECKED, Array.from(set));
    ...
    //scheduler
    let scheduler = await getSchedulerInStorage();
    ...
    function schedulerEvent() {
      ...
      function setShowSchedulerSetting(isShow) {
        const divSchedulerSetting = root.querySelector(
          `#${prefix}div-scheduler-setting`,
        );
        if (divSchedulerSetting) {
          divSchedulerSetting.style.display = isShow ? "block" : "none";
          divSchedulerSetting.style.pointerEvents = isShow ? "auto" : "none";
        }
      }
    ...
    async function onChangeSchedulerType(val) {
      const newSchduler = await getSchedulerInStorage();
      ...
      setSchedulerInStorage(newSchduler);
    }
    ...
    function addButtonsEvent() {
      ...
      function resetAllEvent() {
        try {
          GM_deleteValue(KEY_POST_LENGTH);
          GM_deleteValue(KEY_STOP_TASK);
          GM_deleteValue(KEY_RETRY_CALL);
          GM_deleteValue(KEY_POST);
          GM_deleteValue(KEY_GROUPS_NEED_POST);
          GM_deleteValue(KEY_GROUPS_POSTED);
          GM_deleteValue(KEY_ALL_GROUPS);
          GM_deleteValue(KEY_IS_IN_PROGRESS);
          GM_deleteValue(KEY_IS_TEST);
          GM_setValue(KEY_IS_SCROLL_DETECT_LIST_GROUP, true);
          GM_setValue(KEY_COUNT_RESET_GROUPS, 0);
        } catch (error) {
          showNotify({
      ...
      async function saveCustomMinutesEvent() {
        try {
          ...
          if (inputCustomMinutes) {
            ...
            const isDevMode =
              (await GM_getValue(KEY_IS_DEVELOPER_MODE)) || false;
            val = Number(val);
          ...
          await createSchedulerReloadDashboard();
        } catch (error) {
          showNotify({
            message: error.message,
            type: "error",
      ...
      if (btnGetListGroups) {
        btnGetListGroups.addEventListener("click", async () => {
          try {
            ...
            // GM_setValue(KEY_ALL_GROUPS, allgroups);
            // GM_setValue(KEY_IS_SCROLL_DETECT_LIST_GROUP, false);
            // GM_setValue(KEY_COUNT_RESET_GROUPS, 0);
            // await updateDataSavedInfo();
            // console.log("Get list groups...");
            // GM_openInTab(URL_LIST_GROUPS, { active: true });
            GM_sendMessage(KEY_GET_LIST_GROUPS, { url: URL_LIST_GROUPS });
          } catch (error) {
      ...
      if (btnTest) {
        btnTest.addEventListener("click", async () => {
          //auto test
          await automation({ forceChange: false, isTest: true });
        });
      }
      ...
      if (btnStop) {
        btnStop.addEventListener("click", async () => {
          setProgress(false);
          GM_setValue(KEY_STOP_TASK, true);
        });
      }
      ...
      if (btnAuto) {
        btnAuto.addEventListener("click", async () => {
          //auto
          await automation({ forceChange: false, isTest: false });
        });
      }
          ...
          try {
            ...
            if (maxGroupPerTime) {
              ...
              if (!Number.isNaN(Number(val))) {
                ...
                GM_setValue(KEY_MAX_GROUP_PER_TIME, Number(val));
              } else {
                GM_setValue(KEY_MAX_GROUP_PER_TIME, 1);
              }
            }
            showNotify({
      ...
      if (btnClearGroup) {
        ...
        btnClearGroup.addEventListener("click", () => {
          if (isConfirmingClearGroups) {
            ...
            setDataSavedInStorage([]);
            GM_deleteValue(KEY_INDEXS_GROUP_CHECKED);
            drawListGroups([]);
            showNotify({
      ...
      if (btnChangeTheme) {
        btnChangeTheme.addEventListener("click", async () => {
          const isDarkTheme = (await GM_getValue(KEY_IS_DARK_THEME)) || false;
          const newIsDarkTheme = !isDarkTheme;
          GM_setValue(KEY_IS_DARK_THEME, newIsDarkTheme);
          if (newIsDarkTheme) {
      ...
      try {
        ...
        if (inputPerTime) {
          ...
          inputPerTime.addEventListener("keydown", async (e) => {
            if (e.ctrlKey && e.shiftKey && (e.key === "D" || e.key === "d")) {
              ...
              const devMode =
                (await GM_getValue(KEY_IS_DEVELOPER_MODE)) || false;
              GM_setValue(KEY_IS_DEVELOPER_MODE, !devMode);
              updateDataSavedInfo();
            }
          });
        }
        ...
        if (checkboxIsProcessing) {
          checkboxIsProcessing.addEventListener("change", (e) => {
            ...
            GM_setValue(KEY_IS_IN_PROGRESS, val);
          });
        }
        ...
        if (checkboxIsFixStealFocus) {
          checkboxIsFixStealFocus.addEventListener("change", (e) => {
            const val = e.target.checked;
            GM_setValue(KEY_IS_FIX_STEAL_FOCUS, val);
          });
        }
        ...
        if (checkboxIsScheduler) {
          checkboxIsScheduler.addEventListener("change", (e) => {
            const val = e.target.checked;
            scheduler.isScheduler = val;
            setSchedulerInStorage({ ...scheduler, time: now() });
          });
        }
        ...
        if (checkboxIsReloadDashboard) {
          checkboxIsReloadDashboard.addEventListener("change", async (e) => {
            const val = e.target.checked;
            GM_setValue(KEY_IS_RELOAD_DASHBOARD, val);
            if (val) {
              await createSchedulerReloadDashboard();
            }
          });
